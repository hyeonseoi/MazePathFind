...
public class DFSSolver : MonoBehaviour
{
    ...
    /// <summary>
    /// UI 버튼(OnClick)에서 호출: DFS 알고리즘을 시작합니다.
    /// </summary>
    public void StartDFS()
    {
        ...
        List<MazeCell> path = DFSSearch(grid, startCell, goalCell, width, depth);
        if (path == null)
        {
            UnityEngine.Debug.LogError("DFS: 경로를 찾을 수 없습니다!");
            return;
        }
    ...
    private List<MazeCell> DFSSearch(MazeCell[,] grid, MazeCell start, MazeCell goal, int width, int depth)
    {
    ...
    private IEnumerator MoveAgent(GameObject agent, List<MazeCell> path)
    {
        ...
        foreach (MazeCell cell in path)
        {
            ...
            // DFS 잔상 타일 생성 (타일 타입: DFS)
            TileManager.CreateOrUpdateTile(cell.transform.position, FloorTileType.DFS);
            yield return null;
        }
        stopwatch.Stop();
        timeText.text = "DFS Time: " + stopwatch.Elapsed.TotalSeconds.ToString("F2") + " s";
        Destroy(agent);
    }
